...
export const extractFileContent = <T extends {}>(file: File): Promise<T> => {
...
export const downloadJSONViaDataURL = <T extends {}>(obj: T, fileName: string) => {
...
export const downloadJSON = async <T extends {}>(
	obj: T,
	fileName: string = 'scene' + JSON_FILE_SUFFIX
) => {
	if (obj) {
		...
		if (navigator.canShare && navigator.canShare({ files: [file] })) {
			...
			try {
				...
				downloadJSONViaDataURL<T>(obj, fileName);
			}
		} else {
			downloadJSONViaDataURL<T>(obj, fileName);
		}
